<script lang="ts">
	...
	import { Zap, MapPin, LayoutDashboard, Users, Leaf } from '@lucide/svelte';
	let { data, children }: LayoutProps = $props();
</script>

<!-- Top Navigation -->
<nav class="sticky top-0 z-50 border-b border-border/60 bg-background/80 backdrop-blur-xl">
	<div class="mx-auto flex h-14 max-w-5xl items-center justify-between px-4 md:px-12 xl:px-24">
		...
		<div class="flex items-center gap-1">
			...
			<a href="/consumo" class="flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground">
				<Leaf class="size-3.5" />
				Consumo
			</a>
...
<div class="flex w-full flex-col gap-y-6 px-4 pt-8 pb-24 md:px-12 md:pb-12 xl:px-24">
	{@render children()}
</div>
